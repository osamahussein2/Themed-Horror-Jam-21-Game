...
float DIALOGUE_TEXT2_CHARACTER_SIZE;
...
void GameScene::InitializeDay2()
{
    ...
    DIALOGUE_TEXT2_CHARACTER_SIZE = 50.0f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2);

    dialogueTexts[0].InitializeText("Fonts/Roboto-Regular.ttf", DIALOGUE_TEXT2_CHARACTER_SIZE, false, false,
        sf::Color::White, Vector2f(textPanelX, textPanelY));

    dialogueTexts[1].InitializeText("Fonts/Roboto-Regular.ttf", DIALOGUE_TEXT2_CHARACTER_SIZE, false, false,
        sf::Color::White, Vector2f(textPanelX, textPanelY));

    dialogueTexts[2].InitializeText("Fonts/Roboto-Regular.ttf", DIALOGUE_TEXT2_CHARACTER_SIZE, false, false,
        sf::Color::White, Vector2f(textPanelX, textPanelY));
}
...
void GameScene::UpdateDay2(float deltaTime)
{
    ...
    case GameState::OPERATION_ACTIVE:
    {
        ...
        UpdateDay2OperationScene(deltaTime);
        break;
    }
    ...
    case GameState::SUCCESSFUL_DAY_ACTIVE:
    {
        ...
        if (daySuccessfulTimer < 1.0f)
        {
            ...
            daySuccessfulText.InitializeText("Fonts/Roboto-Regular.ttf", "You passed Day 2!", daySuccessfulCharacterSize,
...
// Update patient here
void GameScene::UpdateDay2Patients()
{
...
void GameScene::UpdateDay2OperationScene(float deltaTime)
{
    // Get mouse position for click detection
    Vector2i mousePixelPos = Mouse::getPosition(*Engine::Instance()->GetWindow());
    Vector2f mousePos = Engine::Instance()->GetWindow()->mapPixelToCoords(mousePixelPos);
