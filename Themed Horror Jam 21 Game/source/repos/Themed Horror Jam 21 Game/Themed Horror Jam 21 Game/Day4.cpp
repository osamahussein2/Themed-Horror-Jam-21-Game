...
float DIALOGUE_TEXT4_CHARACTER_SIZE;
...
void GameScene::InitializeDay4()
{
    ...
    DIALOGUE_TEXT4_CHARACTER_SIZE = 40.0f * (resolution.x / 1920.0f);

    for (int i = 0; i < maxDialogueTexts; i++)
    {
        dialogueTexts[i].InitializeText("Fonts/Roboto-Regular.ttf", DIALOGUE_TEXT4_CHARACTER_SIZE, false, false,
...
void GameScene::UpdateDay4(float deltaTime)
{
    ...
    case GameState::DIALOGUE_ACTIVE:
    {
        ...
        if (Keyboard::isKeyPressed(Keyboard::Key::Enter) && inputCooldown <= 0.0f)
        {
            ...
            else
            {
                ...
                operationScene.InitializeDot({ Vector2f(resolution.x / 1.70f, resolution.y / 3.23f),
                    Vector2f(resolution.x / 2.31f, resolution.y / 3.26f), 
                    Vector2f(resolution.x / 2.40f, resolution.y / 1.94f), 
                    Vector2f(resolution.x / 1.64f, resolution.y / 1.93f), 
                    Vector2f(resolution.x / 1.90f, resolution.y / 11.87f), 
                    Vector2f(resolution.x / 2.02f, resolution.y / 11.87f),
                    Vector2f(resolution.x / 1.81f, resolution.y / 3.43f), 
                    Vector2f(resolution.x / 2.09f, resolution.y / 4.37f) },
                    7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                    3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
        ...
        if (surgeryRoom.OperationTableSprite.getGlobalBounds().contains(mousePos) && isInputEnabled)
        {
            ...
            if (Mouse::isButtonPressed(Mouse::Button::Left) && !mouseClicked)
            {
                ...
                if (!surgeryRoom.IsTimerRunning())
                {
                    ...
                    if (Menu::GetDifficulty() == "Easy") surgeryRoom.StartTimer(4, 31.0f);
                    else if (Menu::GetDifficulty() == "Normal") surgeryRoom.StartTimer(4, 1.0f);
                    else if (Menu::GetDifficulty() == "Hard") surgeryRoom.StartTimer(3, 31.0f);
                }
                mouseClicked = true;
            }
    ...
    case GameState::OPERATION_ACTIVE:
    {
        ...
        UpdateDay4OperationScene(deltaTime);
        break;
    }
    ...
    case GameState::SUCCESSFUL_DAY_ACTIVE:
    {
        ...
        if (daySuccessfulTimer < 1.0f)
        {
            ...
            daySuccessfulText.InitializeText("Fonts/Roboto-Regular.ttf", "You passed Day 4!", daySuccessfulCharacterSize,
...
void GameScene::UpdateDay4Patients()
{
    ...
    case 1: // 2nd patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.maxDots = 9;

            operationScene.InitializeDot({ Vector2f(resolution.x / 1.77f, resolution.y / 3.88f),
                Vector2f(resolution.x / 2.14f, resolution.y / 2.81f), 
                Vector2f(resolution.x / 1.81f, resolution.y / 2.38f), 
                Vector2f(resolution.x / 2.12f, resolution.y / 2.13f), 
                Vector2f(resolution.x / 1.76f, resolution.y / 1.55f), 
                Vector2f(resolution.x / 2.20f, resolution.y / 1.32f), 
                Vector2f(resolution.x / 1.75f, resolution.y / 1.30f), 
                Vector2f(resolution.x / 2.17f, resolution.y / 1.82f), 
                Vector2f(resolution.x / 1.77f, resolution.y / 2.73f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 2: // 3rd patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.maxDots = 11;

            operationScene.InitializeDot({ Vector2f(resolution.x / 1.80f, resolution.y / 3.93f),
                Vector2f(resolution.x / 1.99f, resolution.y / 3.99f),
                Vector2f(resolution.x / 2.22f, resolution.y / 3.58f),
                Vector2f(resolution.x / 2.13f, resolution.y / 2.81f),
                Vector2f(resolution.x / 1.78f, resolution.y / 2.87),
                Vector2f(resolution.x / 1.83f, resolution.y / 2.23f),
                Vector2f(resolution.x / 2.12f, resolution.y / 2.21f),
                Vector2f(resolution.x / 2.18f, resolution.y / 1.80f),
                Vector2f(resolution.x / 1.75f, resolution.y / 1.79f),
                Vector2f(resolution.x / 1.73f, resolution.y / 1.51f),
                Vector2f(resolution.x / 2.25f, resolution.y / 1.50f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 3: // 4th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.25f, resolution.y / 1.38f),
                Vector2f(resolution.x / 2.15f, resolution.y / 1.49f),
                Vector2f(resolution.x / 2.06f, resolution.y / 1.63f),
                Vector2f(resolution.x / 2.00f, resolution.y / 1.78f),
                Vector2f(resolution.x / 1.92f, resolution.y / 2.03),
                Vector2f(resolution.x / 1.86f, resolution.y / 2.44f),
                Vector2f(resolution.x / 1.79f, resolution.y / 3.12f),
                Vector2f(resolution.x / 2.18f, resolution.y / 3.07f),
                Vector2f(resolution.x / 1.75f, resolution.y / 1.33f),
                Vector2f(resolution.x / 1.73f, resolution.y / 1.38f),
                Vector2f(resolution.x / 1.97f, resolution.y / 1.57f),
                Vector2f(resolution.x / 2.00f, resolution.y / 2.03f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 4: // 5th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.maxDots = 13;

            operationScene.InitializeDot({ Vector2f(resolution.x / 2.17f, resolution.y / 3.65f),
                Vector2f(resolution.x / 2.08f, resolution.y / 2.81f),
                Vector2f(resolution.x / 1.97f, resolution.y / 2.24f),
                Vector2f(resolution.x / 1.88f, resolution.y / 1.82f),
                Vector2f(resolution.x / 1.78f, resolution.y / 1.56f),
                Vector2f(resolution.x / 1.71f, resolution.y / 1.42f),
                Vector2f(resolution.x / 2.27f, resolution.y / 1.42f),
                Vector2f(resolution.x / 2.13f, resolution.y / 1.55f),
                Vector2f(resolution.x / 2.04f, resolution.y / 1.70f),
                Vector2f(resolution.x / 1.90f, resolution.y / 2.00f),
                Vector2f(resolution.x / 1.82f, resolution.y / 2.40f),
                Vector2f(resolution.x / 1.78f, resolution.y / 2.98f),
                Vector2f(resolution.x / 1.73f, resolution.y / 3.65f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 5: // 6th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.17f, resolution.y / 4.12f),
                Vector2f(resolution.x / 2.04f, resolution.y / 4.06f),
                Vector2f(resolution.x / 1.87f, resolution.y / 4.11f),
                Vector2f(resolution.x / 1.74f, resolution.y / 4.09f),
                Vector2f(resolution.x / 1.82f, resolution.y / 3.35f),
                Vector2f(resolution.x / 1.88f, resolution.y / 2.92f),
                Vector2f(resolution.x / 1.98f, resolution.y / 2.52f),
                Vector2f(resolution.x / 2.06f, resolution.y / 2.25f),
                Vector2f(resolution.x / 2.15f, resolution.y / 2.03f),
                Vector2f(resolution.x / 2.03f, resolution.y / 2.04f),
                Vector2f(resolution.x / 1.94f, resolution.y / 2.04f),
                Vector2f(resolution.x / 1.84f, resolution.y / 2.04f),
                Vector2f(resolution.x / 1.78f, resolution.y / 2.03f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 6: // 7th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.22f, resolution.y / 1.32f),
                Vector2f(resolution.x / 2.12f, resolution.y / 1.32f),
                Vector2f(resolution.x / 1.95f, resolution.y / 1.32f),
                Vector2f(resolution.x / 1.84f, resolution.y / 1.31f),
                Vector2f(resolution.x / 1.75f, resolution.y / 1.31f),
                Vector2f(resolution.x / 1.72f, resolution.y / 1.41f),
                Vector2f(resolution.x / 1.72f, resolution.y / 1.55f),
                Vector2f(resolution.x / 1.73f, resolution.y / 1.72f),
                Vector2f(resolution.x / 1.76f, resolution.y / 1.86f),
                Vector2f(resolution.x / 2.20f, resolution.y / 1.84f),
                Vector2f(resolution.x / 2.24f, resolution.y / 1.72f),
                Vector2f(resolution.x / 2.27f, resolution.y / 1.60f),
                Vector2f(resolution.x / 2.27f, resolution.y / 1.42f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 7: // 8th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.50f, resolution.y / 1.56f),
                Vector2f(resolution.x / 2.39f, resolution.y / 2.01f),
                Vector2f(resolution.x / 2.30f, resolution.y / 2.79f),
                Vector2f(resolution.x / 2.13f, resolution.y / 4.27f),
                Vector2f(resolution.x / 1.94f, resolution.y / 5.51f),
                Vector2f(resolution.x / 1.75f, resolution.y / 4.03f),
                Vector2f(resolution.x / 1.69f, resolution.y / 2.90f),
                Vector2f(resolution.x / 1.66f, resolution.y / 2.02f),
                Vector2f(resolution.x / 1.59f, resolution.y / 1.56f),
                Vector2f(resolution.x / 2.11f, resolution.y / 3.13f),
                Vector2f(resolution.x / 2.04f, resolution.y / 2.73f),
                Vector2f(resolution.x / 1.91f, resolution.y / 2.70f),
                Vector2f(resolution.x / 1.82f, resolution.y / 3.11f),
                Vector2f(resolution.x / 1.95f, resolution.y / 1.43f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 8: // 9th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.24f, resolution.y / 1.37f),
                Vector2f(resolution.x / 2.09f, resolution.y / 1.36f),
                Vector2f(resolution.x / 1.92f, resolution.y / 1.36f),
                Vector2f(resolution.x / 1.80f, resolution.y / 1.35f),
                Vector2f(resolution.x / 1.72f, resolution.y / 1.35f),
                Vector2f(resolution.x / 1.78f, resolution.y / 2.11f),
                Vector2f(resolution.x / 1.86f, resolution.y / 2.12f),
                Vector2f(resolution.x / 1.95f, resolution.y / 2.11f),
                Vector2f(resolution.x / 2.06f, resolution.y / 2.11f),
                Vector2f(resolution.x / 2.15f, resolution.y / 2.11f),
                Vector2f(resolution.x / 2.55f, resolution.y / 1.35f),
                Vector2f(resolution.x / 2.52f, resolution.y / 1.49f),
                Vector2f(resolution.x / 1.57f, resolution.y / 1.34f),
                Vector2f(resolution.x / 1.58f, resolution.y / 1.48f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 9: // 10th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.00f, resolution.y / 5.54f),
                Vector2f(resolution.x / 1.90f, resolution.y / 5.48f),
                Vector2f(resolution.x / 1.90f, resolution.y / 41.54f),
                Vector2f(resolution.x / 1.96f, resolution.y / 41.54f),
                Vector2f(resolution.x / 2.01f, resolution.y / 41.54f),
                Vector2f(resolution.x / 2.06f, resolution.y / 18.95f),
                Vector2f(resolution.x / 1.97f, resolution.y / 18.95f),
                Vector2f(resolution.x / 1.90f, resolution.y / 17.14f),
                Vector2f(resolution.x / 1.85f, resolution.y / 17.42f),
                Vector2f(resolution.x / 1.89f, resolution.y / 10.69f),
                Vector2f(resolution.x / 1.96f, resolution.y / 10.69f),
                Vector2f(resolution.x / 2.03f, resolution.y / 10.69f),
                Vector2f(resolution.x / 1.92f, resolution.y / 8.06f),
                Vector2f(resolution.x / 2.00f, resolution.y / 7.66f),
                Vector2f(resolution.x / 1.95f, resolution.y / 4.58f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 10: // 11th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.maxDots = 16;

            operationScene.InitializeDot({ Vector2f(resolution.x / 1.70f, resolution.y / 3.82f),
                Vector2f(resolution.x / 1.70f, resolution.y / 3.28f),
                Vector2f(resolution.x / 1.70f, resolution.y / 2.92f),
                Vector2f(resolution.x / 1.69f, resolution.y / 2.54f),
                Vector2f(resolution.x / 2.29f, resolution.y / 3.96f),
                Vector2f(resolution.x / 2.31f, resolution.y / 3.44f),
                Vector2f(resolution.x / 2.32f, resolution.y / 2.86f),
                Vector2f(resolution.x / 2.34f, resolution.y / 2.47f),
                Vector2f(resolution.x / 2.15f, resolution.y / 3.54f),
                Vector2f(resolution.x / 2.10f, resolution.y / 3.01f),
                Vector2f(resolution.x / 2.01f, resolution.y / 2.73f),
                Vector2f(resolution.x / 1.89f, resolution.y / 2.73f),\
                Vector2f(resolution.x / 1.79f, resolution.y / 3.06f),
                Vector2f(resolution.x / 1.79f, resolution.y / 3.69f),
                Vector2f(resolution.x / 1.96f, resolution.y / 1.35f),
                Vector2f(resolution.x / 1.97f, resolution.y / 1.48f), },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 11: // 12th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 1.95f, resolution.y / 5.45f),
                Vector2f(resolution.x / 1.95f, resolution.y / 4.58f),
                Vector2f(resolution.x / 1.95f, resolution.y / 3.71f),
                Vector2f(resolution.x / 1.95f, resolution.y / 3.17f),
                Vector2f(resolution.x / 1.95f, resolution.y / 2.77f),
                Vector2f(resolution.x / 1.95f, resolution.y / 2.36f),
                Vector2f(resolution.x / 1.96f, resolution.y / 2.11f),
                Vector2f(resolution.x / 1.96f, resolution.y / 1.88f),
                Vector2f(resolution.x / 1.96f, resolution.y / 1.71f),
                Vector2f(resolution.x / 1.97f, resolution.y / 1.55f),
                Vector2f(resolution.x / 1.97f, resolution.y / 1.44f),
                Vector2f(resolution.x / 1.97f, resolution.y / 1.35f),
                Vector2f(resolution.x / 2.15f, resolution.y / 4.24f),
                Vector2f(resolution.x / 2.27f, resolution.y / 3.74f),
                Vector2f(resolution.x / 1.80f, resolution.y / 4.14f),
                Vector2f(resolution.x / 1.70f, resolution.y / 3.53f),
                Vector2f(resolution.x / 2.03f, resolution.y / 4.35f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 12: // 13th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 2.63f, resolution.y / 1.47f),
                Vector2f(resolution.x / 2.50f, resolution.y / 1.47f),
                Vector2f(resolution.x / 1.60f, resolution.y / 1.45f),
                Vector2f(resolution.x / 1.55f, resolution.y / 1.46f),
                Vector2f(resolution.x / 1.60f, resolution.y / 1.58f),
                Vector2f(resolution.x / 2.48f, resolution.y / 1.60f),
                Vector2f(resolution.x / 2.17f, resolution.y / 2.43f),
                Vector2f(resolution.x / 2.10f, resolution.y / 2.42f),
                Vector2f(resolution.x / 2.03f, resolution.y / 2.41f),
                Vector2f(resolution.x / 1.96f, resolution.y / 2.39f),
                Vector2f(resolution.x / 1.87f, resolution.y / 2.39f),
                Vector2f(resolution.x / 1.78f, resolution.y / 2.38f),
                Vector2f(resolution.x / 1.79f, resolution.y / 2.12f),
                Vector2f(resolution.x / 1.87f, resolution.y / 2.11f),
                Vector2f(resolution.x / 1.94f, resolution.y / 2.11f),
                Vector2f(resolution.x / 2.05f, resolution.y / 2.11f),
                Vector2f(resolution.x / 2.15f, resolution.y / 2.11f),
                Vector2f(resolution.x / 2.18f, resolution.y / 1.95f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 13: // 14th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.maxDots = 19;

            operationScene.InitializeDot({ Vector2f(resolution.x / 2.51f, resolution.y / 1.57f),
                Vector2f(resolution.x / 2.46f, resolution.y / 1.71f),
                Vector2f(resolution.x / 2.42f, resolution.y / 1.93f),
                Vector2f(resolution.x / 2.36f, resolution.y / 2.21f),
                Vector2f(resolution.x / 2.31f, resolution.y / 2.63f),
                Vector2f(resolution.x / 1.61f, resolution.y / 1.59f),
                Vector2f(resolution.x / 1.62f, resolution.y / 1.74f),
                Vector2f(resolution.x / 1.65f, resolution.y / 1.94f),
                Vector2f(resolution.x / 1.67f, resolution.y / 2.24f),
                Vector2f(resolution.x / 1.69f, resolution.y / 2.62f),
                Vector2f(resolution.x / 2.17f, resolution.y / 2.42f),
                Vector2f(resolution.x / 2.14f, resolution.y / 2.22f),
                Vector2f(resolution.x / 2.16f, resolution.y / 2.02f),
                Vector2f(resolution.x / 1.77f, resolution.y / 2.43f),
                Vector2f(resolution.x / 1.80f, resolution.y / 2.19f),
                Vector2f(resolution.x / 1.77f, resolution.y / 1.95f),
                Vector2f(resolution.x / 2.12f, resolution.y / 1.28f),
                Vector2f(resolution.x / 1.81f, resolution.y / 1.27f),
                Vector2f(resolution.x / 1.80f, resolution.y / 3.48f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
    ...
    case 14: // 15th patient
        if (!operationSceneChanged)
        {
            ...
            operationScene.InitializeDot({ Vector2f(resolution.x / 1.96f, resolution.y / 15.43f),
                Vector2f(resolution.x / 1.96f, resolution.y / 10.38f),
                Vector2f(resolution.x / 2.56f, resolution.y / 1.39f),
                Vector2f(resolution.x / 1.74f, resolution.y / 3.70f),
                Vector2f(resolution.x / 2.03f, resolution.y / 4.84f),
                Vector2f(resolution.x / 1.87f, resolution.y / 4.01f),
                Vector2f(resolution.x / 2.22f, resolution.y / 3.96f),
                Vector2f(resolution.x / 2.34f, resolution.y / 2.90f),
                Vector2f(resolution.x / 1.77f, resolution.y / 2.87f),
                Vector2f(resolution.x / 1.99f, resolution.y / 3.59f),
                Vector2f(resolution.x / 2.15f, resolution.y / 2.45f),
                Vector2f(resolution.x / 1.79f, resolution.y / 2.24f),
                Vector2f(resolution.x / 1.97f, resolution.y / 1.89f),
                Vector2f(resolution.x / 1.88f, resolution.y / 1.56f),
                Vector2f(resolution.x / 1.77f, resolution.y / 1.29f),
                Vector2f(resolution.x / 2.20f, resolution.y / 1.29f),
                Vector2f(resolution.x / 2.02f, resolution.y / 1.31f),
                Vector2f(resolution.x / 1.96f, resolution.y / 1.44f),
                Vector2f(resolution.x / 2.22f, resolution.y / 1.56f),
                Vector2f(resolution.x / 1.74f, resolution.y / 1.70f) },
                7.5f * (resolution.x / 1920.0f), Color::Red, Color::Red,
                3.75f * (((resolution.x / 1920.0f) + (resolution.y / 1080.0f)) / 2));
...
void GameScene::UpdateDay4OperationScene(float deltaTime)
{
    // Get mouse position for click detection
    Vector2i mousePixelPos = Mouse::getPosition(*Engine::Instance()->GetWindow());
    Vector2f mousePos = Engine::Instance()->GetWindow()->mapPixelToCoords(mousePixelPos);
